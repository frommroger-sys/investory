...
def build_pdf(out_path, logo_bytes, report):
    ...
    def p_link(txt, url): return Paragraph(
        f"<bullet>&#8226;</bullet>{txt} <font color='#0b5bd3'><u><link href='{url}'>Quelle</link></u></font>", bullet)
    ...
    for s in report.get("headline", [])[:5]: story.append(p_bullet(s))
    ...
    def region(title_txt, data):
        ...
        for txt,url in data.get("news", [])[:8]: story.append(p_link(f"<b>Unternehmensnews:</b> {txt}", url))
...
def wp_upload_media(file_path, title=None, alt_text=None):
    ...
    with open(file_path, "rb") as f:
        ...
        r = requests.post(f"{WP_BASE}/wp-json/wp/v2/media",
                          headers=headers, files=files, auth=(WP_USERNAME, WP_APP_PASSWORD), timeout=60)
    r.raise_for_status()
